...
/// This instruction Returns the element in an Coop Vector at specified index
struct DxilInst_CoopVector_Index {
  llvm::Instruction *Instr;
  // Construction and identification
  DxilInst_CoopVector_Index(llvm::Instruction *pInstr) : Instr(pInstr) {}
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(Instr,
                                          hlsl::OP::OpCode::CoopVector_Index);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
  ...
  // Accessors
  llvm::Value *get_coopvector() const { return Instr->getOperand(1); }
  void set_coopvector(llvm::Value *val) { Instr->setOperand(1, val); }
  llvm::Value *get_index() const { return Instr->getOperand(2); }
  void set_index(llvm::Value *val) { Instr->setOperand(2, val); }
};

/// This instruction Annotate a wave matrix pointer with the type information
struct DxilInst_CoopVector_Annotate {
  llvm::Instruction *Instr;
  // Construction and identification
  DxilInst_CoopVector_Annotate(llvm::Instruction *pInstr) : Instr(pInstr) {}
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(
        Instr, hlsl::OP::OpCode::CoopVector_Annotate);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
  ...
  enum OperandIdx {
    arg_coopvectorPtr = 1,
    arg_coopvectorprops = 2,
  };
  // Accessors
  llvm::Value *get_coopvectorPtr() const { return Instr->getOperand(1); }
  void set_coopvectorPtr(llvm::Value *val) { Instr->setOperand(1, val); }
  llvm::Value *get_coopvectorprops() const { return Instr->getOperand(2); }
  void set_coopvectorprops(llvm::Value *val) { Instr->setOperand(2, val); }
};

/// This instruction Fill coopvector with scalar value
struct DxilInst_CoopVector_Fill {
  llvm::Instruction *Instr;
  // Construction and identification
  DxilInst_CoopVector_Fill(llvm::Instruction *pInstr) : Instr(pInstr) {}
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(Instr,
                                          hlsl::OP::OpCode::CoopVector_Fill);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
/// This instruction Perform scalar operation on each element of Cooperative
/// Vector
struct DxilInst_CoopVector_ScalarOp {
  llvm::Instruction *Instr;
  // Construction and identification
  DxilInst_CoopVector_ScalarOp(llvm::Instruction *pInstr) : Instr(pInstr) {}
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(
        Instr, hlsl::OP::OpCode::CoopVector_ScalarOp);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
struct DxilInst_CoopVector_ArithmeticOp {
  ...
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(
        Instr, hlsl::OP::OpCode::CoopVector_ArithmeticOp);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
struct DxilInst_CoopVector_LoadRawBuf {
  ...
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(
        Instr, hlsl::OP::OpCode::CoopVector_LoadRawBuf);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
struct DxilInst_CoopVector_LoadGroupShared {
  llvm::Instruction *Instr;
  // Construction and identification
  DxilInst_CoopVector_LoadGroupShared(llvm::Instruction *pInstr)
      : Instr(pInstr) {}
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(
        Instr, hlsl::OP::OpCode::CoopVector_LoadGroupShared);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
struct DxilInst_CoopVector_StoreRawBuf {
  ...
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(
        Instr, hlsl::OP::OpCode::CoopVector_StoreRawBuf);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
struct DxilInst_CoopVector_StoreGroupShared {
  llvm::Instruction *Instr;
  // Construction and identification
  DxilInst_CoopVector_StoreGroupShared(llvm::Instruction *pInstr)
      : Instr(pInstr) {}
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(
        Instr, hlsl::OP::OpCode::CoopVector_StoreGroupShared);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
struct DxilInst_CoopVector_MatMul {
  ...
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(Instr,
                                          hlsl::OP::OpCode::CoopVector_MatMul);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
/// This instruction Vector Matrix Multiply
struct DxilInst_CoopVector_MatMulAdd {
  llvm::Instruction *Instr;
  // Construction and identification
  DxilInst_CoopVector_MatMulAdd(llvm::Instruction *pInstr) : Instr(pInstr) {}
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(
        Instr, hlsl::OP::OpCode::CoopVector_MatMulAdd);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
/// This instruction Perform scalar operation on each element of Cooperative
/// Vector
struct DxilInst_CoopVector_CopyFrom {
  llvm::Instruction *Instr;
  // Construction and identification
  DxilInst_CoopVector_CopyFrom(llvm::Instruction *pInstr) : Instr(pInstr) {}
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(
        Instr, hlsl::OP::OpCode::CoopVector_CopyFrom);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
  ...
  // Operand indexes
  enum OperandIdx {
    arg_coopvectorPtrTo = 1,
    arg_coopvectorPtrFrom = 2,
  };
  // Accessors
  llvm::Value *get_coopvectorPtrTo() const { return Instr->getOperand(1); }
  void set_coopvectorPtrTo(llvm::Value *val) { Instr->setOperand(1, val); }
  llvm::Value *get_coopvectorPtrFrom() const { return Instr->getOperand(2); }
  void set_coopvectorPtrFrom(llvm::Value *val) { Instr->setOperand(2, val); }
};

/// This instruction Perform scalar operation on each element of Cooperative
/// Vector
struct DxilInst_CoopVector_ScalarMulAdd {
  llvm::Instruction *Instr;
  // Construction and identification
  DxilInst_CoopVector_ScalarMulAdd(llvm::Instruction *pInstr) : Instr(pInstr) {}
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(
        Instr, hlsl::OP::OpCode::CoopVector_ScalarMulAdd);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
  ...
  llvm::Value *get_coopvectorPtr() const { return Instr->getOperand(1); }
  void set_coopvectorPtr(llvm::Value *val) { Instr->setOperand(1, val); }
  llvm::Value *get_valueToMul() const { return Instr->getOperand(2); }
  void set_valueToMul(llvm::Value *val) { Instr->setOperand(2, val); }
  llvm::Value *get_valueToAdd() const { return Instr->getOperand(3); }
  void set_valueToAdd(llvm::Value *val) { Instr->setOperand(3, val); }
};
...
struct DxilInst_CoopVector_ScalarMin {
  ...
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(
        Instr, hlsl::OP::OpCode::CoopVector_ScalarMin);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
struct DxilInst_CoopVector_ScalarMax {
  ...
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(
        Instr, hlsl::OP::OpCode::CoopVector_ScalarMax);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
struct DxilInst_CoopVector_ReadFromIndex {
  llvm::Instruction *Instr;
  // Construction and identification
  DxilInst_CoopVector_ReadFromIndex(llvm::Instruction *pInstr)
      : Instr(pInstr) {}
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(
        Instr, hlsl::OP::OpCode::CoopVector_ReadFromIndex);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
/// This instruction Write a Coop Vector Element at Index
struct DxilInst_CoopVector_WriteToIndex {
  llvm::Instruction *Instr;
  // Construction and identification
  DxilInst_CoopVector_WriteToIndex(llvm::Instruction *pInstr) : Instr(pInstr) {}
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(
        Instr, hlsl::OP::OpCode::CoopVector_WriteToIndex);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
/// This instruction Perform scalar operation on each element of Cooperative
/// Vector
struct DxilInst_CoopVector_Activation {
  llvm::Instruction *Instr;
  // Construction and identification
  DxilInst_CoopVector_Activation(llvm::Instruction *pInstr) : Instr(pInstr) {}
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(
        Instr, hlsl::OP::OpCode::CoopVector_Activation);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
/// This instruction Perform scalar operation on each element of Cooperative
/// Vector
struct DxilInst_CoopVector_Clamp {
  llvm::Instruction *Instr;
  // Construction and identification
  DxilInst_CoopVector_Clamp(llvm::Instruction *pInstr) : Instr(pInstr) {}
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(Instr,
                                          hlsl::OP::OpCode::CoopVector_Clamp);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
  ...
  enum OperandIdx {
    arg_coopvectorPtr = 1,
    arg_coopvectorPtrFlr = 2,
    arg_coopvectorPtrCeil = 3,
  };
  // Accessors
  llvm::Value *get_coopvectorPtr() const { return Instr->getOperand(1); }
  void set_coopvectorPtr(llvm::Value *val) { Instr->setOperand(1, val); }
  llvm::Value *get_coopvectorPtrFlr() const { return Instr->getOperand(2); }
...
/// This instruction Perform scalar clamp operation on each element of
/// Cooperative Vector
struct DxilInst_CoopVector_ScalarClamp {
  ...
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(
        Instr, hlsl::OP::OpCode::CoopVector_ScalarClamp);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
/// This instruction Perform bitwise operation on each element of Cooperative
/// Vector
struct DxilInst_CoopVector_BitwiseOp {
  llvm::Instruction *Instr;
  // Construction and identification
  DxilInst_CoopVector_BitwiseOp(llvm::Instruction *pInstr) : Instr(pInstr) {}
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(
        Instr, hlsl::OP::OpCode::CoopVector_BitwiseOp);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
  ...
  llvm::Value *get_coopvectorPtrOtherVec() const {
    return Instr->getOperand(3);
  }
  void set_coopvectorPtrOtherVec(llvm::Value *val) {
    Instr->setOperand(3, val);
  }
};

/// This instruction Perform Scalar bitwise operation on each element of
/// Cooperative Vector
struct DxilInst_CoopVector_ScalarBitwiseOp {
  llvm::Instruction *Instr;
  // Construction and identification
  DxilInst_CoopVector_ScalarBitwiseOp(llvm::Instruction *pInstr)
      : Instr(pInstr) {}
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(
        Instr, hlsl::OP::OpCode::CoopVector_ScalarBitwiseOp);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
struct DxilInst_CoopVector_BitwiseShift {
  llvm::Instruction *Instr;
  // Construction and identification
  DxilInst_CoopVector_BitwiseShift(llvm::Instruction *pInstr) : Instr(pInstr) {}
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(
        Instr, hlsl::OP::OpCode::CoopVector_BitwiseShift);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
  ...
  llvm::Value *get_num() const { return Instr->getOperand(3); }
  void set_num(llvm::Value *val) { Instr->setOperand(3, val); }
};

/// This instruction Unary Complement
struct DxilInst_CoopVector_Negate {
  llvm::Instruction *Instr;
  // Construction and identification
  DxilInst_CoopVector_Negate(llvm::Instruction *pInstr) : Instr(pInstr) {}
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(Instr,
                                          hlsl::OP::OpCode::CoopVector_Negate);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
struct DxilInst_CoopVector_OuterProductAccumulate {
  llvm::Instruction *Instr;
  // Construction and identification
  DxilInst_CoopVector_OuterProductAccumulate(llvm::Instruction *pInstr)
      : Instr(pInstr) {}
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(
        Instr, hlsl::OP::OpCode::CoopVector_OuterProductAccumulate);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
...
struct DxilInst_CoopVector_ReduceSumAccumulate {
  llvm::Instruction *Instr;
  // Construction and identification
  DxilInst_CoopVector_ReduceSumAccumulate(llvm::Instruction *pInstr)
      : Instr(pInstr) {}
  operator bool() const {
    return hlsl::OP::IsDxilOpFuncCallInst(
        Instr, hlsl::OP::OpCode::CoopVector_ReduceSumAccumulate);
  }
  // Validation support
  bool isAllowed() const { return true; }
  bool isArgumentListValid() const {
